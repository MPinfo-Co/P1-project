...
> 「Repo 結構細節」，請參考：[repo-design.md](P1-project/docs/repo-design.md)
...
- 執行工作如需產生計畫檔，一律產生在根目錄 `P1-project/docs/` 下
...
每次 `git pull` P1-code 後，執行以下指令更新本機依賴，避免因新套件未安裝導致後端無法啟動：

```bash
# 後端
cd P1-code/backend && .venv/bin/python -m pip install -r requirements.txt
```
...
> 「Issue body內文」，請參考：[issue-body-spec.md](P1-project/docs/issue-body-spec.md)
> 「自動產生檔案的內文格式」，請參考：[auto-file-format.md](P1-project/docs/auto-file-format.md)
---
## SA 流程
...
- 前後端路徑或格式有差異時，**以後端為準（sdSpec 規格為依據），前端配合調整**。
- 例外：若後端明顯有誤（不符合 sdSpec），先確認後再決定改哪一側，不可自行假設。
...
1. 先讀 [workflow_guide.md](P1-project/docs/workflow_guide.md)，了解各階段觸發流程
...
凡工作內容涉及 GitHub Actions workflow（`.yml`）、AI Agent prompt（`prompts/*.md`）或兩者串接關係，**動手前先讀** [workflow_guide.md](P1-project/docs/workflow_guide.md)，確認 yml → prompt 對應結構後再執行。
