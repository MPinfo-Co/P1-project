...
當人類成員說「我們來處理 PG Issue #99」這類的指示時，開始實作前：

1. 先讀取對應的 issue body，需要的資訊及相關連結都在 body 之中，請以其中的資訊為主，不要發散，遇到問題詢問人類成員
2. 從 issue body 的「分支」欄位取得 branch 名稱，切換到該 branch 並執行 `git pull`。Branch 由 GitHub Actions 自動建立，**不手動建立**。（P1-project 例外，PM 可直接在 main 上操作）
...
**產出**
1. `business-logic.md` — 背景與目的、需求說明、商業邏輯、資料模型示意、SD 注意事項、畫面示意
2. `SD-WBS.md` — 格式如下，類型限定 `Schema`、`API`、`畫面`、`其他`，須完整涵蓋本次所有變更
...
> **TDD（Technical Design Document）**：包含設計說明與測試案例，作為 PG 實作依據，以 Issue 為單位保存。

**讀取**
1. `business-logic.md` — 商業邏輯背景（由 SA 填寫）
2. `SD-WBS.md` — 本次 SD 工作範圍（由 SA 填寫）

**產出**

依工作範圍完成：`Prototype/`、`Spec/`、`schema/`（如有異動）、`TDD/issue-{N}.md`
