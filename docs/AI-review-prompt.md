...
1. **讀取審查清單**：讀取 `P1-project/docs/AI-review-doclist.md`，取得目標文件與每份文件的「上次審查」日期
...
5. **增量判斷**：對每份文件以 git diff 比對「上次審查」日期，填入「本次變動」欄位（是/否）：
   - **上次審查 ≤ 3 天 且 本次變動＝否** → 沿用上次結論，不重新審查，「上次審查」日期不更新
   - **其餘情況** → 重新審查，「上次審查」日期更新為今天
