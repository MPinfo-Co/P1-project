...
**AI-review-prompt.md**：Phase 4 步驟編號跳號：今日移除「建議修改清單」後步驟變為 `1, 2, 4, 5`，跳過 3。（事實差異，今日 prompt 改版引入）
...
**FunctionList.md**：完成狀態欄位全部空白（11 個功能均無填寫 `未開始`／`進行中`／`已完成`），文件標題已宣告欄位用途但未實際維護，讀者無法從此文件判斷功能完成度。（優化）
