...
|---|------|---------|------|---------|------|------------|
| 1 | v8 | 事實差異 | 低 | P1-project/CLAUDE.md | P1-project 目錄樹 | 移除 `AI-CONTEXT.md` 一行（檔案不存在） |
| 2 | v8 | 事實差異 | 低 | P1-project/CLAUDE.md | P1-code 目錄樹 | 移除根目錄 `tests/` 一行（目錄不存在） |
| 3 | v8 | 事實差異 | 中 | P1-project/docs/repo-design.md | P1-analysis 目錄樹 | 補上 `references/` 目錄（存在且含 10+ 重要參考文件） |
| 4 | v8 | 事實差異 | 低 | P1-project/docs/repo-design.md | P1-project 目錄樹 | 補上 `CLAUDE.md` 與 `PRD.md` |
| 5 | v8 | 事實差異 | 低 | P1-project/docs/repo-design.md | P1-code 目錄樹 | 移除根目錄 `tests/` 與 `docs/` 兩行（均不存在） |
| 6 | v8 | 事實差異 | 低 | P1-project/docs/repo-design.md | VersionDiff 格式範例 | `LeavePage.tsx` 改為 `LeavePage.jsx` |
| 7 | v8 | 事實差異 | 低 | P1-project/docs/workflow/spec/c-workflow.md | backend-ci 說明 | 移除「根目錄 `tests/` 整合測試不在 CI 執行範圍」（目錄已不存在） |
| 8 | v7 | 事實差異 | 低 | P1-analysis/README.md | 流程圖第 4 行 | `（PG／AI` 補上閉括號 → `（PG／AI）` |
| 9 | v7 | 優化 | 低 | P1-code/SETUP.md | 第 5 節「範例格式」行 | `` `{type}: 工作說明` `` 改為 `` `{type}({scope}): 說明` `` |
| 10 | v2 | 事實差異 | 低 | P1-project/README.md | 四個 Repo 表格 P1-code 列 | `React/TypeScript` 改為 `React/JavaScript` |
| 11 | v2 | 事實差異 | 低 | P1-project/docs/workflow/quick-start.md | PG 第 5 步「撰寫程式碼與測試」 | `React/TypeScript .tsx` 改為 `React/JavaScript .jsx` |
| 12 | v2 | 事實差異 | 低 | P1-project/docs/repo-design.md | P1-code 目錄樹 `frontend/` 標註 | `React + TypeScript` 改為 `React + JavaScript` |
