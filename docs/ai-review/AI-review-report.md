...
**README.md**：
- **[待處理]** 重要文件索引中 SETUP.md（第 14 行）連結指向 `P1-code/SETUP.md`（GitHub 絕對 URL），但該檔案不存在。開發環境準備指引實際位於 `P1-project/docs/setup.md`，應更正 Repo 與路徑。（事實差異，續自 v31）
- **[待處理]** techStack.md 連結使用同 Repo（P1-project）的 GitHub 絕對 URL `https://github.com/MPinfo-Co/P1-project/blob/main/docs/techStack.md`，同表其他 P1-project 文件均使用本機相對路徑，應改為 `docs/techStack.md`。（優化建議）

**CLAUDE.md**：
- **[待處理]** PG 流程「讀取」步驟 5 的 techStack.md 連結使用同 Repo（P1-project）的 GitHub 絕對 URL，同文件其他連結（repo-design.md、workflow_guide.md 等）均使用本機相對路徑，應改為 `docs/techStack.md`。（優化建議）

**issue-body-spec.md**：
- **[待處理]** SA / SD / PG Issue body 分別標示由 `p-workflow.yml`、`a-workflow.yml`、`d-workflow.yml` 產生，但這三個 workflow 檔案已不存在（已改名）。實際檔案為 `wf_epic_to_sa.yml`（P1-project）、`wf_sa_to_sd.yml`（P1-design）、`wf_sd_to_pg.yml`（P1-design）。檔案名稱與 GitHub 連結皆須更新。（事實差異，續自 v31）

**repo-design.md**：
- **[待處理]** P1-project 目錄結構樹狀圖未列出 `coding-rule-backend.md`、`coding-rule-frontend.md`、`setup.md`。這三份檔案已存在且被 README.md 索引或審查清單收錄，應補入樹狀圖。（事實差異，續自 v31）

**spec-guide.md**：
- **[待處理]** 目錄結構樹狀圖使用 `{fn_功能}/` 作為目錄佔位符，暗示所有功能目錄帶有 `fn_` 前綴，但命名規則表已正確記載為 `{功能編號}`（範例含 `fn_user/` 和 `auth/`）。樹狀圖應改為 `{功能編號}/` 以與下方命名規則表一致。（事實差異，續自 v31）

**setup.md**：
- **[待處理]** 第 139 行「每次 push 到 `issue-*` branch 或開 PR 時，CI 自動執行」，但 `wf_pg_ci.yml` 實際觸發條件為 push 到 `pg-*` branch，應更正分支 pattern。（事實差異，續自 v31）

**AI-review-doclist.md**：
- **[待處理]** `setup.md` 和 `techStack.md` 連結使用同 Repo（P1-project）的 GitHub 絕對 URL，同表其他 P1-project 文件均使用本機相對路徑，應分別改為 `../setup.md` 和 `../techStack.md` 以維持一致性。（優化建議，v31 僅含 setup.md，本次擴充含 techStack.md）

**coding-rule-frontend.md**：
- **[待處理]** 「二、架構規則」章節序號不連續且順序錯誤：2-1 → 2-2 → 2-3 → 2-5 → 2-6 → 2-4。Section 2-4「Token 取得方式」排在 2-6 之後，應重新編號或調整順序。（優化，續自 v31）

#### P1-design（審查 1 份文件，1 份有問題）

**functionList.md**：
- **[待處理]** 功能清單中功能編號 `auth` 與 Spec 實際目錄名稱 `Spec/fn_auth/` 不一致（目錄內檔案亦使用 `fn_auth` 前綴，如 `fn_auth_00_overview.md`）。文件規定「目錄名稱 = 功能編號」，兩者應統一。（事實差異，續自 v31）

### 摘要
> 本次審查 14 份文件，發現 9 份有問題。

（v31 的 8 項問題均未解決，全部續列。新增 2 項優化建議：README.md 和 CLAUDE.md 的 techStack.md 連結使用同 Repo 絕對 URL。AI-review-doclist.md 優化建議擴充含 techStack.md。）

### 未發現問題的文件
> 以下 5 份文件未發現問題：workflow_guide.md、auto-file-format.md、techStack.md、coding-rule-backend.md、AI-review-prompt.md。
