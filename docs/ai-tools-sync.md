...
MP-Box 專案的 AI 審查工具（skill + agents）統一由 P1-project repo 維護，  
...
```
/ai-work-review (告知是哪個repo的哪個issue,ex:[PG]xxx#{n})
```

工具會自動判斷是 SA / SD / PG 階段，  
...
在本機修改工具後，告知 Claude code 下列資訊，將工具同步到 repo 讓全員取得：
...
repo 有更新時，告知 Claude code 下列資訊，將工具拉回本機套用：
