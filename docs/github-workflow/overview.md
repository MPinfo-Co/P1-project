...
## GitHub Projects 看板設定建議

GitHub Projects 聚合四個 Repo 的 Issue，以下設定建議讓 PM 在多 Epic 並行時快速定位瓶頸。

每個 View 都是同一份資料的不同「篩選視角」，不需要額外維護——系統根據 Issue 的標籤與狀態自動顯示。

**建議 View 設定：**

| View 名稱 | 類型 | 分組方式 | 篩選條件 | 用途 |
|----------|------|---------|---------|------|
| Epic 全覽 | Board | 以 Epic 標籤分組 | 無 | 快速看各 Epic 整體進度 |
| 進行中（今日） | Board | 以負責角色分組 | `status: in-progress` | 每日站會用，看誰在做什麼 |
| 瓶頸追蹤 | Table | 以 Repo 分組 | `label: blocked` | 快速找出卡住的 Issue |
| 待審查 | Table | 以審查人分組 | `label: needs-review` | 確認 PR 審查積壓情況 |

**建議 Label 規範（跨 Repo 統一）：**
- `blocked`：等待外部依賴或決策
- `needs-review`：PR 已開，等待審查
- `needs-sa-review`：SD 發現需求問題，等待 SA/PM 確認
- `on-hold`：暫緩，不是緊急問題

> 五個以上 Epic 同時進行時，建議 PM 每天開 Projects「瓶頸追蹤」view，優先處理 `blocked` Issue，再確認各 Epic 的下一個待辦項目是否有人負責。
...
**[→ quick-start.md](quick-start.md)**：按角色列出第一天的操作步驟（含指令範例與常見卡關），不需要先讀完所有文件。

### 第二步：理解規範全貌（閱讀順序）

加入 P1 開發團隊的新成員，建議依以下順序深入閱讀規範文件：

1. **本文（overview.md）**：理解四個 Repo 的職責分工與四個核心機制
2. **repo-design.md**：熟悉自己負責的 Repo 的目錄結構與文件格式
3. **workflow.md 對應章節**：依自己的角色只需重點閱讀對應章節
   - SA：第二節（系統分析）
   - SD：第三節（系統設計）+ 活文件衝突處理 + Schema.md 衝突處理
   - PG：第四節（系統開發）+ AI 自主實作資訊讀取指引
   - PM：第一節（工作分派）+ 第五節（PM 介入機制）
4. **workflow.md 第九節（例外處理）**：上手後再閱讀，了解特殊情境的處置方式

> 新成員第一個 Issue 建議由有經驗的同角色成員陪同走一遍流程，確認分支建立、PR 填寫、審查流程都操作正確。若無同角色資深成員可陪同，由 PM 擔任引導人。
