...
**Goal:** 在 `P1-project/docs/frontend-guide.md` 撰寫一份教學型前端說明文件，以主流程走讀方式解釋 P1-code 前端各檔案的串連關係與技術框架選型。

**Architecture:** 純文件產出，不涉及程式碼變更。文件以「使用者操作主流程」為主線，從程式啟動 → 登入 → 主畫面 → 事件清單 → 事件詳情，沿途解釋遇到的框架與設計決策，末尾附技術速查表。

**Tech Stack:** React 18、React Router v6、MUI v6、Zustand、React Context、Vite — 均為說明對象，不需安裝。

**Spec:** `docs/superpowers/specs/2026-04-18-frontend-guide-design.md`
...
| 動作 | 路徑 |
|------|------|
| 新增 | `P1-project/docs/frontend-guide.md` |
...
**Files:**
- Create: `P1-project/docs/frontend-guide.md`

- [ ] **Step 1：建立檔案，寫入目錄骨架**

在 `P1-project/docs/frontend-guide.md` 寫入以下內容（後續 Task 將逐章填充）：
...
```bash
ls P1-project/docs/frontend-guide.md
```
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: scaffold frontend-guide.md"
```

---

### Task 2：第 1 章 — 這份文件怎麼讀

**Files:**
- Modify: `P1-project/docs/frontend-guide.md`

- [ ] **Step 1：在骨架後追加第 1 章內容**
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 1 - how to read this guide"
```
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 2 - tech stack overview"
```
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 3.1 - program startup"
```
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 3.2 - login flow"
```

---

### Task 6：第 3.3 章 — 進入主畫面
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 3.3 - main layout"
```

---

### Task 7：第 3.4 章 — 瀏覽事件清單
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 3.4 - issue list"
```

---

### Task 8：第 3.5 章 — 開啟事件詳情
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 3.5 - issue detail"
```
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 4 - state management design"
```

---

### Task 10：第 5 章 — 技術框架速查表
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 5 - tech reference"
```

---

### Task 11：第 6 章 — 目前尚未完成的功能
...
```bash
git -C P1-project add docs/frontend-guide.md
git -C P1-project commit -m "docs: add chapter 6 - work in progress"
```
...
**Files:**
- Review: `P1-project/docs/frontend-guide.md`

- [ ] **Step 1：瀏覽完整文件，確認以下事項**

```bash
cat P1-project/docs/frontend-guide.md
```
...
- [ ] 沒有 TBD 或空白章節
- [ ] 流程圖的箭頭格式一致（`A → B → C`）
- [ ] 第 4 章有說明 IssuesContext 現狀（mock 資料，未被主頁面使用）

- [ ] **Step 2：Push 到遠端**

```bash
git -C P1-project push origin main
```

Expected: 推送成功，GitHub 上可看到 `docs/frontend-guide.md`。
