# Backend Syntax Guide 實作計劃

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 撰寫 `docs/learn/backend-syntax-guide.md`，逐一說明 `backend-guide.md` 每個程式碼片段中出現的語法，讓「會基本 Python、未接觸 FastAPI 生態系」的初學者看懂。

**Architecture:** 單一 Markdown 文件，開頭放語法索引表（語法 | 簡易說明 | 出現章節 | anchor），本體章節順序對應 backend-guide.md 有程式碼的章節（§4.1→§4.6、§6、§7），每個語法條目固定四欄格式。

**Tech Stack:** Markdown、繁體中文
...
### `語法名稱`
**是什麼：** 一句話定義
**專案範例：** （直接引用 backend-guide.md 的程式碼片段）
**白話解釋：** 口語說明（可用比喻）
...
**Files:**
- Create: `docs/learn/backend-syntax-guide.md`

- [ ] **Step 1：建立文件，寫入開頭與章節骨架**
...
確認有 8 個章節標題（4.1、4.2、4.3、4.4、4.5、4.6、6、7）和 28 個索引條目。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 建立 backend-syntax-guide 骨架與語法索引表"
```

---

## Task 2：撰寫 §4.1 應用程式啟動

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.1 應用程式啟動` 區段）

- [ ] **Step 1：填入 §4.1 的兩個語法條目**

在 `## 4.1 應用程式啟動` 與下一個 `---` 之間填入：
...
確認 `## 4.1 應用程式啟動` 下有 `app.include_router` 和 `CORSMiddleware` 兩個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.1 應用程式啟動語法說明"
```

---

## Task 3：撰寫 §4.2 登入與 JWT 認證

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.2 使用者登入與 JWT 認證` 區段）

- [ ] **Step 1：填入 §4.2 的五個語法條目**

在 `## 4.2 使用者登入與 JWT 認證` 與下一個 `---` 之間填入：
...
確認 `## 4.2 使用者登入與 JWT 認證` 下有：`@router.post/@router.get`、`Depends`、`bcrypt`、`jwt`、`db.query` 共五個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.2 登入與 JWT 認證語法說明"
```

---

## Task 4：撰寫 §4.3 Flash Task

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.3 排程任務：Flash Task` 區段）

- [ ] **Step 1：填入 §4.3 的四個語法條目**

在 `## 4.3 排程任務：Flash Task` 與下一個 `---` 之間填入：
...
確認 `## 4.3 排程任務：Flash Task` 下有：`@celery_app.task`、`beat_schedule`、`httpx.AsyncClient`、list slicing 共四個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.3 Flash Task 語法說明"
```

---

## Task 5：撰寫 §4.4 AI 快速分析

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.4 AI 快速分析（Claude Haiku）` 區段）

- [ ] **Step 1：填入 §4.4 的三個語法條目**

在 `## 4.4 AI 快速分析（Claude Haiku）` 與下一個 `---` 之間填入：
...
確認 `## 4.4 AI 快速分析（Claude Haiku）` 下有：`client.messages.create`、system/user message、`json.loads` 共三個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.4 AI 快速分析語法說明"
```

---

## Task 6：撰寫 §4.5 AI 日報彙整

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.5 AI 日報彙整（Claude Sonnet）` 區段）

- [ ] **Step 1：填入 §4.5 的兩個語法條目**

在 `## 4.5 AI 日報彙整（Claude Sonnet）` 與下一個 `---` 之間填入：
...
確認 `## 4.5 AI 日報彙整（Claude Sonnet）` 下有：`db.add/db.merge`、`db.commit/db.refresh` 共兩個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.5 AI 日報彙整語法說明"
```

---

## Task 7：撰寫 §4.6 前端查詢事件

**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 4.6 前端查詢事件` 區段）

- [ ] **Step 1：填入 §4.6 的四個語法條目**

在 `## 4.6 前端查詢事件` 與下一個 `---` 之間填入：
...
確認 `## 4.6 前端查詢事件` 下有：`Query`、`class Schema(BaseModel)`、`response_model`、`Optional[str]` 共四個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §4.6 前端查詢事件語法說明"
```
...
**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 6. 配置管理` 區段）
...
確認 `## 6. 配置管理` 下有：`class Settings(BaseSettings)`、`settings.DATABASE_URL`、`.env + Field` 共三個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §6 配置管理語法說明"
```
...
**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（`## 7. 測試架構` 區段）
...
確認 `## 7. 測試架構` 下有：`@pytest.fixture`、`conftest.py`、`TestClient`、`dependency_overrides`、`assert response.status_code` 共五個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 新增 §7 測試架構語法說明"
```
...
**Files:**
- Modify: `docs/learn/backend-syntax-guide.md`（確認完整性）

- [ ] **Step 1：驗證語法條目總數**

```bash
grep -c "^### " docs/learn/backend-syntax-guide.md
```
...
```bash
grep "^### " docs/learn/backend-syntax-guide.md
```

預期輸出（依序）：
```
§4.1（2）：app.include_router、CORSMiddleware
§4.2（5）：@router.post/@router.get、Depends、bcrypt、jwt、db.query
§4.3（4）：@celery_app.task、beat_schedule、httpx.AsyncClient、list slicing
§4.4（3）：client.messages.create、system/user message、json.loads
§4.5（2）：db.add/db.merge、db.commit/db.refresh
§4.6（4）：Query、class Schema(BaseModel)、response_model、Optional[str]
§6（3）：class Settings(BaseSettings)、settings.DATABASE_URL、.env+Field
§7（5）：@pytest.fixture、conftest.py、TestClient、dependency_overrides、assert
```

- [ ] **Step 3：最終 Commit 並 Push**

```bash
git add docs/learn/backend-syntax-guide.md
git commit -m "docs: 完成 backend-syntax-guide.md 全文"
git push
```
