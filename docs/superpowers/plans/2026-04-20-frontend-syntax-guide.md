# Frontend Syntax Guide 實作計劃

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 撰寫 `docs/learn/frontend-syntax-guide.md`，逐一說明 `frontend-guide.md` 每個程式碼片段中出現的語法，讓「會基本 JS、未接觸 React 生態系」的初學者看懂。

**Architecture:** 單一 Markdown 文件，開頭放語法索引表（語法 → anchor），本體章節順序完全對應 frontend-guide.md（3.1 → 3.2 → 3.3 → 3.4 → 3.5 → 4），每個語法條目固定四欄格式。
...
實作時每個語法一律用以下結構，不可省略任何欄位：

```markdown
### `語法名稱`
**是什麼：** 一句話定義
**專案範例：** （直接引用 frontend-guide.md 的程式碼片段）
**白話解釋：** 口語說明（可用比喻）
...
**Files:**
- Create: `docs/learn/frontend-syntax-guide.md`

- [ ] **Step 1：建立文件，寫入開頭與章節骨架**

內容如下（索引表的 anchor 欄位留空，Task 8 填入）：
...
| 語法 | 出現章節 | 跳到說明 |
|------|----------|----------|
| `createRoot().render()` | 3.1 程式啟動 | [連結](#createroot-render) |
...
用文字編輯器打開 `docs/learn/frontend-syntax-guide.md`，確認：
- 索引表有 40 個語法條目
- 七個章節標題（索引表 + 六個章節）都存在
...
```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 建立 frontend-syntax-guide 骨架與語法索引表"
```
...
**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 3.1 程式啟動` 區段）

- [ ] **Step 1：填入 §3.1 的六個語法條目**

在 `## 3.1 程式啟動` 與下一個 `---` 之間填入：
...
開啟文件確認 `## 3.1 程式啟動` 下方有：`createRoot`、`StrictMode`、`ThemeProvider`、`CssBaseline`、`createTheme`、`styleOverrides` 共六個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §3.1 程式啟動語法說明"
```
...
**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 3.2 登入流程` 區段）

- [ ] **Step 1：填入 §3.2 的九個語法條目**

在 `## 3.2 登入流程` 與下一個 `---` 之間填入：
...
- `await` 只能用在 `async` 函式內部，不能在一般函式或頂層（模組頂層除外）使用
...
開啟文件確認 `## 3.2 登入流程` 下方有：`async/await`、`fetch`、`JSON.stringify`、`res.status/res.json`、`localStorage`、`set()`、`ProtectedRoute`、三元運算子、`<Navigate>` 共九個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §3.2 登入流程語法說明"
```
...
**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 3.3 進入主畫面` 區段）

- [ ] **Step 1：填入 §3.3 的七個語法條目**

在 `## 3.3 進入主畫面` 與下一個 `---` 之間填入：
...
確認 `## 3.3 進入主畫面` 下有：`<Routes>/<Route>`、巢狀路由、`<Box sx>`、`<Outlet>`、`<NavLink>+isActive`、`Object.entries...`、`?.` 與 `??` 共七個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §3.3 進入主畫面語法說明"
```

---

## Task 5：撰寫 §3.4 瀏覽事件清單

**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 3.4 瀏覽事件清單` 區段）

- [ ] **Step 1：填入 §3.4 的六個語法條目**

在 `## 3.4 瀏覽事件清單` 與下一個 `---` 之間填入：
...
**白話解釋：** 每次元件重新渲染，函式都會重新建立（記憶體位址不同）。`useCallback` 讓 `fetchEvents` 在 `applied` 和 `page` 沒變時保持同一個參考，避免 `useEffect` 以為「函式變了！要重跑！」造成無限迴圈。
...
確認 `## 3.4 瀏覽事件清單` 下有：`useState`、`useCallback`、`useEffect`、`e.stopPropagation`、`<Chip>`、`<Popover>` 共六個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §3.4 瀏覽事件清單語法說明"
```

---

## Task 6：撰寫 §3.5 開啟事件詳情

**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 3.5 開啟事件詳情` 區段）

- [ ] **Step 1：填入 §3.5 的六個語法條目**

在 `## 3.5 開啟事件詳情` 與下一個 `---` 之間填入：
...
確認 `## 3.5 開啟事件詳情` 下有：`useParams`、`<Tabs>/<Tab>`、條件渲染 `&&`、兩步驟 async、`transition`、`<Collapse>` 共六個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §3.5 開啟事件詳情語法說明"
```
...
**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（`## 4. 狀態管理設計` 區段）

- [ ] **Step 1：填入 §4 的六個語法條目**

在 `## 4. 狀態管理設計` 與最後一個 `---` 之間填入：
...
確認 `## 4. 狀態管理設計` 下有：`create()`、`useXxxStore`、`createContext`、`<Context.Provider>`、`useContext`、自訂 hook 共六個 `###` 標題。

- [ ] **Step 3：Commit**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 新增 §4 狀態管理設計語法說明"
```
...
**Files:**
- Modify: `docs/learn/frontend-syntax-guide.md`（確認索引表 anchor 正確）

- [ ] **Step 1：驗證索引表所有 anchor 可跳轉**

檢查索引表的每個 `[連結](#anchor)` 對應的 `###` 標題是否存在。若有不一致（標題改過但 anchor 未更新），手動修正。

Markdown anchor 規則：標題文字轉小寫、空格換 `-`、特殊符號移除。例如：
- `### \`useState\`` → anchor 是 `#usestate`
- `### \`res.status\` / \`res.json()\`` → anchor 是 `#resstatus--resjson`

- [ ] **Step 2：確認語法總數**

文件內應有以下語法條目（共 40 個），逐一核對：

§3.1（6 個）：`createRoot`、`StrictMode`、`ThemeProvider`、`CssBaseline`、`createTheme`、`styleOverrides`

§3.2（9 個）：`async/await`、`fetch`、`JSON.stringify`、`res.status/res.json`、`localStorage`、Zustand `set`、`ProtectedRoute`、三元運算子、`<Navigate>`

§3.3（7 個）：`<Routes>/<Route>`、巢狀路由、`<Box sx>`、`<Outlet>`、`<NavLink>+isActive`、`Object.entries...`、`?.` 與 `??`

§3.4（6 個）：`useState`、`useCallback`、`useEffect`、`e.stopPropagation`、`<Chip>`、`<Popover>`

§3.5（6 個）：`useParams`、`<Tabs>/<Tab>`、條件渲染 `&&`、兩步驟 async、`transition`、`<Collapse>`

§4（6 個）：`create()`、`useXxxStore`、`createContext`、`<Context.Provider>`、`useContext`、自訂 hook

- [ ] **Step 3：最終 Commit 並 Push**

```bash
git add docs/learn/frontend-syntax-guide.md
git commit -m "docs: 完成 frontend-syntax-guide.md 全文"
git push
```
