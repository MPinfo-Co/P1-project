...
## 使用 AI 工具（Claude Code）

P1 使用兩層 CLAUDE.md 分別管理團隊規範與個人偏好：

| 檔案 | 對象 | 用途 |
|------|------|------|
| `P1-project/CLAUDE.md` | 全團隊 | 團隊規範，**不要修改** |
| 根目錄 `CLAUDE.md`（個人） | 個人 | 個人偏好覆蓋層，自行維護 |

**AI 載入順序**：Claude Code 先載入根目錄 CLAUDE.md，再載入子目錄 CLAUDE.md，後者優先。
因此個人設定若與團隊規範衝突，**團隊規範優先**。

**個人 CLAUDE.md 只寫與團隊不同的設定**，例如：
...
不要在個人 CLAUDE.md 複製貼上團隊規範內容，這樣只會造成混淆，且實際上是低優先級。
